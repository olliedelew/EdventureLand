  ...
  if((operator3 == "/") && (((answer1/answer2/answer3) % answer4) !== 0) && (operator2 == "/")){
    ...
    if (equation_case == 1) {
      ...
      if((operator1 == '*' && operator2 == '*' && operator3 == '*') || (operator1 == '/' && operator2 == '/' && operator3 == '/')){
        ...
        equation.innerHTML = `${answer1} ${operator1} ((${answer2} ${operator2} ${answer3}) ${operator3} <input type="number" id="fill_answer" placeholder="?"\>) = ${solution}`;
      } else if (operator1 == '/' && operator3 == '/' || operator1 == '/' && operator3 == '*' || operator1 == '*' && operator3 == '*' && operator2 != '/' || operator1 == '*' && operator3 == '/') {
        equation.innerHTML = `(${answer1} ${operator1} ${answer2}) ${operator2} (${answer3} ${operator3} <input type="number" id="fill_answer" placeholder="?"\>) = ${solution}`;
       } else if (operator1 == '/') {
        equation.innerHTML = `(${answer1} ${operator1} ${answer2}) ${operator2} ${answer3} ${operator3} <input type="number" id="fill_answer" placeholder="?"\> = ${solution}`;
       } else if (operator2 == '/') {
...
submit.onclick = function () {
  ...
  if (empty_input == "yes") {
    ...
    if(answers.length % 5 == 0 && testtime >= 5){
      let roundNo = parseInt(document.getElementById("round").innerHTML.split(" ")[1]);
      roundNo += 1;
      document.getElementById("round").innerHTML = '<b>Round ' + roundNo + '</b>';
      testtime = testtime - 2;
...
function startTimer(dur) {
  ...
  if (check == null) {
    check = setInterval(function () {
      ...
      // if the next timer tick is less than 0
      if (--timer < 0) {
...
function endGame(){
  ...
  retry.style.display = 'flex';
    ...
    score_hidden.value = points_number.innerHTML;
    correct_ones.value = correct.length;
    incorrect_ones.value = incorrectAns.length;
    difficulty_sent_off.value = difficulty;
    opps.value = document.getElementById('operators').value.split(', ').join('_');
    ...
    for (let i = 0; i < answers.length; i++) {
      ...
      for (let j = 0; j < answers[i].length; j++) {
        if(answers[i][j].slice(-1) == "C"){
          // Show up answer with green square border around input
          ...
          // Show up answer with red square border around input
...
window.onload = function() {
  ...
  // Start at round 1
  document.getElementById("round").innerHTML = '<b>Round 1</b>';
  // Timer starts at 14 to account for the second delay
  startTimer(14);
